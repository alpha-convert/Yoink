...
class DataflowGraph:
    ...
    def __call__(self, *args):
        ...
        If first argument is a Yoink instance, re-trace the function for composition.
        ...
        # Check if first arg is a Yoink instance
        if isinstance(args[0], Yoink):
            if len(args) != len(self.input_types) + 1:
                raise ValueError(f"Expected {len(self.input_types) + 1} arguments (yoink + {len(self.input_types)} streams), got {len(args)}")
    ...
    def to_graphviz(self):
        from yoink.util.viz_builder import VizBuilder
